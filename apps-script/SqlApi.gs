...
;invs.forEach(inv=>{try {const currency=String(inv.Currency||"RM").toUpperCase();if (currency!=="RM"&&currency!=="MYR") throw new Error("Foreign currency needs an exchange rate before SQL upload.");
let c=cm[ckey(inv["Customer Name"])];if (!c) throw new Error("Customer record is missing.");
...
const f=findInv(q.invoiceId),inv=f.inv,s=settings();if (!isRmCurrency(inv.Currency)) throw new Error("Only RM invoices can create OR automatically for now.");
...
const items=allItems.filter(x=>x["Invoice ID"]===inv["Invoice ID"]&&Number(x.Amount)!==0);if (!items.length) throw new Error("No money rows are available for SQL upload.");
return {dockey:0,docno:"",docdate:date,postdate:date,taxdate:date,code:c["SQL Customer Code"],companyname:inv["Customer Name"],address1:a[0]||"",address2:a[1]||"",address3:a[2]||"",address4:a[3]||"",country:pick(s,"DEFAULT_COUNTRY","MY"),phone1:phone(inv["Customer Phone"]),agent:pick(s,"DEFAULT_AGENT","----"),project:pick(s,"DEFAULT_PROJECT","----"),terms:inv.Terms||pick(s,"DEFAULT_TERMS","C.O.D."),currencycode:"",currencyrate:"1.00",description:"LeVince Chauffeur Service",cancelled:false,docamt:sqlMoney_(inv.Total),localdocamt:sqlMoney_(inv.Total),docref1:String(inv["Internal Invoice No"]),tin:inv.TIN||"",idtype:Number(inv["ID Type"]||0),idno:inv["ID No"]||"",submissiontype:Number(pick(s,"DEFAULT_SUBMISSION_TYPE",17)),changed:true,sdsdocdetail:items.map((x,i)=>sqlDetail_(x,i+1,s))}
...
const amount=sqlMoney_(doc.docamt||inv.Total);const project=pick(s,"DEFAULT_PROJECT","----"),agent=pick(s,"DEFAULT_AGENT","----");
